function [E,F, q] = poly_long_div_v2(num,den,PredictionHorizon)
...
E_nbr_lines = size(Ec,1);
E_nbr_cols = size(Ec{end},2);

E = zeros(E_nbr_lines, E_nbr_cols);
for i=1:E_nbr_lines
    E(i,:) = [Ec{i,1} zeros(1, E_nbr_cols - size(Ec{i,1},2))];
end

F_nbr_lines = size(Fc,1);
idx = find(Fc{end,1} ~= 0);

F_nbr_cols = size(Fc{end,1}(idx(1):end),2);

F = zeros(F_nbr_lines, F_nbr_cols);
for i=1:F_nbr_lines
    idx_tmp = find(Fc{i,1} ~= 0);
    
    if( (F_nbr_cols-size(Fc{i,1}(idx_tmp:end)) > 0))
        F(i,:) = [Fc{i,1}(idx_tmp:end) zeros(1, F_nbr_cols-size(Fc{i,1}(idx_tmp:end),2))]
    else
        F(i,:) = Fc{i,1}(idx_tmp:end);
    end
end
